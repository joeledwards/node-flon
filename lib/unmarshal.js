...
function unmarshal (input, output) {
  return new Promise((resolve, reject) => {
    ...
    .node('*', (value, path, ancestors) => {
      ...
      if (value === null) {
        output.write(`${assemble(path)} = ${yellow('null')};\n`)
      } else {
        const type = typeof value
        if (type === 'boolean') {
          output.write(`${assemble(path)} = ${purple(value)};\n`)
        } else if (type === 'number') {
          output.write(`${assemble(path)} = ${orange(value)};\n`)
        } else if (type === 'string') {
          output.write(`${assemble(path)} = "${green(esc(value))}";\n`)
        }
      } 
